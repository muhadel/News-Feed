...
import { mainMenuItems } from '@/layouts/main-layout/main-menu-items';
    ...
    <div className="mb-auto">
      ...
      <ul className="pb-12">
        ...
        {items.map((item, index) => {
          ...
          return (
            <Fragment key={'sortable-menu' + t(item.name) + '-' + index}>
                                ...
                                {dropdownItem?.subMenuItems?.map(
                                  (subMenuItem, index) => {
                                    ...
                                    return (
                                      <Menu.Item
                                        key={
                                          'sub-menu' + subMenuItem?.name + index
                                        }
                                        className="px-0 py-0"
                                      >
                                        <Link
...
function MenuLink({ item, isChildActive }: MenuItemsProps) {
  ...
  const { state } = useFilterControls<any, any>({});

  const router = useRouter();

  const handleMenuClick = (subMenu: string) => {
    let path = `${routes.newsFeed.articles}?query=${subMenu}&page=1`;
    if (state.language) path += `&language=${state.language}`;
    router.push(path);
  };
  ...
  return (
    <div
      onClick={async () => {
        handleMenuClick(t(item.name));
      }}
      className={cn(
        'relative flex w-full cursor-pointer items-center justify-between rounded-md px-3.5 py-2 font-medium capitalize text-gray-900',
        isChildActive
